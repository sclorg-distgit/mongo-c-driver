%{?scl:%scl_package mongo-c-driver}
...
BuildRequires: %{?scl_prefix}libbson-devel > %{bsonver}
...
%description
%{pkg_name} is a client library written in C for MongoDB.



%package libs
Summary:    Shared libraries for %{pkg_name}
Group:      Development/Libraries


%description libs
This package contains the shared libraries for %{pkg_name}.
...
for i in examples/*.c examples/*/*.c; do
  install -Dpm 644 $i %{buildroot}%{_datadir}/doc/%{pkg_name}/$i
done

: Rename documentation to match subpackage name
mv %{buildroot}%{_datadir}/doc/%{pkg_name} \
   %{buildroot}%{_datadir}/doc/%{pkg_name}-devel
